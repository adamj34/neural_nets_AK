...
class FeedForward(nn.Module):
    """ a simple linear feed-forward layer followed by non-linearity"""
...
class Block(nn.Module):
    ...
    def forward(self, x):  # x + => residual connection
        x = x + self.sa(self.ln1(x)) # divergence from the original paper: we apply layer norm before the self-attention
...
class GPT(nn.Module):
    ...
    def __init__(self):
        ...
        self.lm_head = nn.Linear(n_embd, vocab_size) # goes from token embeddings to logits
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            # crop idx to the last blocak_size tokens
            idx = idx[:, -block_size:] # (B, T)
            # get the predictions
            logits, loss = self(idx)
            # print(logits.shape) # e.g [1, 488, 65]
